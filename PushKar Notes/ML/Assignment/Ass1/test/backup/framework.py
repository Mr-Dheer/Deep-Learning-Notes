...
#        result =  (np.exp(x) - np.exp(-x))/(np.exp(x)+np.exp(-x))
    ...
    def backward(self, error):
        ...
        d_x = np.multiply(1-np.square(tanh_x),error)
...
class Sigmoid:
    ...
    def backward(self, error):
        ...
        ## Chck vector multiplication sizes ans output shape.
        ## Implement
        d_x = np.multiply(np.multiply(sigmoid_x,(1-sigmoid_x)),error)
        ## End
...
class Linear:
    ...
    def backward(self, error):
        ...
        dW = self.var["W"] - lr*np.matmul(x.T,error)
        #        db = self.var["b"] - lr*np.mean(error,0)
        db = self.var["b"] - lr*np.sum(error,0)
        d_inputs = np.matmul(error,self.var["W"].T)
...
#            module = self.modules[module_index]
    ...
    def backward(self, error):
        ...
        for module_index in reversed(range(len(self.modules))):
            ...
            ## Implement
            var_grad,inp_grad = module.backward(error)
            module_variable_grad = var_grad 
            module_input_grad = inp_grad
...
class MSE:
    def forward(self, prediction, target):
        ...
        diff = T-Y
        meanError = np.matmul(diff.T,diff)/(2*n)
        self.saved_variables={"prediction": Y,"target":T}
        ## End
        return meanError
...
# TO-Do weight update here with the learning rate and not the hard-coded way as done in class func definition
def train_one_step(model, loss, learning_rate, inputs, targets):
    ## Implement
    error = loss.forward(model.forward(inputs),targets)
    variable_grads,_ = model.backward(loss.backward())
    #pdb.set_trace()
    for idx,m in enumerate(model.modules):
        if not hasattr(m,'var'):
            continue
        m.var["W"] = variable_grads["mod_%d.W" % (idx)]
        m.var["b"] = variable_grads["mod_%d.b" % (idx)]
## End
    return error
