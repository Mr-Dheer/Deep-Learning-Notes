...
def verify(images, targets):
    ...
    vpred_label = np.argmax(pred,1)
    num_ok = len(np.where(vpred_label==targets)[0])
...
def validate():
    ...
    accu,count = verify(train_validation_set.images[validation_indices],
                        train_validation_set.labels[validation_indices])
    ## End
...
def test():
    ...
    for i in range(0, len(test_set), batch_size):
        ...
        vAcc,Vcount = verify(images,labels)
        accu+=vAcc
        count+=Vcount
...
for epoch in range(1000):
    ## Implement
    ## TO-DO:: advised to look at the shape to compute count etc... check if neccessary for this befor submitting thr assignment
    vloss = 0
    for i in range(0,len(train_indices),batch_size):
        input = train_validation_set.images[i:i+batch_size]
        labels = train_validation_set.labels[i:i+batch_size]
        pred = model.forward(input)
        vloss += loss.forward(pred,labels)
        var_grads,_ = model.backward(loss.backward())
        for idx,m in enumerate(model.modules):
            if not hasattr(m,'var'):
                continue
            m.var["W"] = m.var["W"] - learning_rate*var_grads["mod_%d.W" % (idx)]
            m.var["b"] = m.var["b"] - learning_rate*var_grads["mod_%d.b" % (idx)]
