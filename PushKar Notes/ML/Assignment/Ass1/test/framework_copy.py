...
class Tanh:
    ...
    def backward(self, error):
        ...
        d_x = np.multiply(np.square(tanh_x),error)
...
class Sigmoid:
    ...
    def backward(self, error):
        ...
        ## Implement
        d_x = np.multiply(np.multiply(sigmoid_x,(sigmoid_x-1)),error)
        ## End
...
class Linear:
    ...
    def backward(self, error):
        ...
        dW = self.var["W"] - np.matmul(x.T,error)
        db = self.var["b"] - np.mean(error,0)
...
class MSE:
    def forward(self, prediction, target):
        ...
        meanError = np.matmul((Y-T).T,Y-T)/n
    ...
    def backward(self):
        ...
        prediction = self.saved_variables["prediction"]
        y = self.saved_variables["target"]
        d_prediction = 2*(prediction-y)/prediction.size
        ## End
